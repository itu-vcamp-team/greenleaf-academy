...
async def get_optional_user(
    ...
    payload = TokenService.decode_token(token)
    if not payload:
        return User(id=uuid.uuid4(), role=UserRole.GUEST, username="Guest", email="guest@local", full_name="Guest User", password_hash="")
    ...
    return user or User(id=uuid.uuid4(), role=UserRole.GUEST, username="Guest", email="guest@local", full_name="Guest User", password_hash="")


def require_roles(allowed_roles: list[UserRole]):
    """
    Dependency to restrict access based on user roles.
    Example: Depends(require_roles([UserRole.ADMIN]))
    """
...
async def get_current_admin(current_user: User = Depends(get_current_user)) -> User:
    """Shortcut to require ADMIN or EDITOR role."""
